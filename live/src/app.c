/* AutoEDO Live — self-contained realtime EDO pitch corrector.
   ...
   Config persists to ~/.autoedo.json (or the configured path; each instance
   of a multi-instance rig gets its own file). The same code runs standalone
   (main.c, a device backend) and embedded in a host process (embed.c, the
   embed backend) -- only main() and the backend differ (see app.h). */
...
struct AeApp
{
    ...
    /* Lock-free mirror of `engine` for an embedding host's audio thread
    ...
    /* The control server ("" port 0 = none) and the pump thread that feeds
       its WebSocket clients; both owned here so the standalone process and
       an embedding host get the same lifecycle. */
    AeHttpServer   *server;
...
/* Caller must hold app->lock. Take the engine out of the live mirror and
   let an embedding host's audio thread turn a block over before it is
   stopped -- a pointer read from ae_app_engine_live() stays valid for the
   duration of one process call (the same grace idiom the sample loader
   uses). Device backends have no host audio thread; the wait is cheap
   against a device open either way. */
static void engine_teardown_locked (App *app)
{
    if (app->engine == NULL)
        return;
    atomic_store_explicit (&app->engine_live, NULL, memory_order_release);
...
static void engine_restart_locked (App *app)
{
    ...
    atomic_store_explicit (&app->engine_live, app->engine, memory_order_release);
}
...
static void status_refresh (App *app)
{
    ...
    snprintf (buf, sizeof (buf),
        "{\"running\":%s,\"embedded\":%s,\"engineBuild\":\"%s\",\"error\":\"%s\","
        ...
        ae_audio_backend_embedded () ? "true" : "false", AE_BUILD_ID, err,
...
static void *pump_thread_fn (void *arg)
{
    ...
    while (! atomic_load_explicit (&app->follow_quit, memory_order_relaxed))
    {
        ...
        if (app->server != NULL)
        {
            ...
            pthread_mutex_unlock (&app->status_lock);
            ae_http_ws_broadcast (app->server, frame, n);
        }
        ae_sleep_ms (100);
    }
    return NULL;
...
AeApp *ae_app_create (const AeAppOptions *opt, char *err, size_t err_len)
{
    ...
    app->port = opt->http_port;
    config_defaults (app);
    if (opt->config_file != NULL && opt->config_file[0] != '\0')
        snprintf (app->config_file, sizeof (app->config_file), "%s",
                  opt->config_file);

    config_load (app);
    /* The host's audio format is a fact about the process, not a setting:
       it rides the engine config where only the embed backend reads it,
       and no JSON key can move it. */
    ...
    engine_restart_locked (app);
    pthread_mutex_unlock (&app->lock);
    /* An engine failure is not fatal: the control surface still comes up so
       the user can pick a working device. */
    ...
    {
        char herr[256];
        app->server = ae_http_start (app->port, handle_request, app,
                                     herr, sizeof (herr));
        if (app->server == NULL)
            /* Not fatal even embedded: the audio path is the headline and
               must not die with the panel. The caller reads
               ae_app_http_running() and decides what to say. */
            fprintf (stderr, "autoedo: web server failed: %s\n", herr);
    }

    app->pump_thread_up =
        pthread_create (&app->pump_thread, NULL, pump_thread_fn, app) == 0;
    return app;
}
...
void ae_app_destroy (AeApp *app)
{
    if (app == NULL)
        return;
    atomic_store_explicit (&app->follow_quit, true, memory_order_relaxed);
    if (app->pump_thread_up)
        pthread_join (app->pump_thread, NULL);
    if (app->follow_thread_up)
        pthread_join (app->follow_thread, NULL);
    if (app->tap_thread_up)
        pthread_join (app->tap_thread, NULL);
    if (app->server != NULL)
        ae_http_stop (app->server);
    pthread_mutex_lock (&app->lock);
    engine_teardown_locked (app);
    pthread_mutex_unlock (&app->lock);
    pthread_mutex_destroy (&app->lock);
    pthread_mutex_destroy (&app->status_lock);
    free (app);
}

bool ae_app_http_running (const AeApp *app)
{
    return app != NULL && app->server != NULL;
}
...
void ae_app_post_config (AeApp *app, const char *json)
{
    ...
    api_config_post (app, json, &resp);
    free (resp.body); /* the echo; an embedding host reads ae_app_status */
}
...
int ae_app_status (AeApp *app, char *out, size_t cap)
{
    if (cap == 0)
        return 0;
    pthread_mutex_lock (&app->status_lock);
    const int n = snprintf (out, cap, "%s", app->status_json);
    pthread_mutex_unlock (&app->status_lock);
...
{
    return atomic_load_explicit (&app->engine_live, memory_order_acquire);
}
